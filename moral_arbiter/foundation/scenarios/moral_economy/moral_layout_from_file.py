        ...
        if self._moral_theory == "amoral":
            ...
            for agent in self.world.agents:
                curr_optimization_metric[agent.idx] = rewards.learned_reward(
                    ...
                    num_houses=np.array(
                        [np.sum(np.array(self.world.maps.get("House", owner=True)) == agent.idx)] +  
                        [np.sum(np.array(self.world.maps.get("House", owner=True)) ==a.idx) for a in self.world.agents if a.idx != agent.idx]
                    ),
                    labor_coefficient=labor_coefficient,
                    isoelastic_eta=isoelastic_eta,
