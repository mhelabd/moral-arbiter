...
def steal_str(all_steals, agent, income=True):
    if income:
        p = [x["income"] for x in all_steals if x["stealing_agent"] == agent]
    else:
        p = [x["income"] for x in all_steals if x["stolen_from_agent"] == agent]
    if len(p) > 0:
        return "{:6.2f} (n={:3d})".format(np.mean(p), len(p))
    else:
        tmp = "~" * 8
...
def report(c_trades, all_builds, all_steals, n_agents, a_indices=None):
    ...
    print(full_steal_str(all_steals, a_indices, income=False))
    print("")
    print(full_steal_str(all_steals, a_indices, income=True))
...
def breakdown(log, remap_key=None):
    ...
    all_steals = []
    for t, steals in enumerate(log.get("Steal", [])):
        if isinstance(steals, dict):
            steals_ = steals["steals"]
        else:
            steals_ = steals
        for steal in steals_:
            this_steal = {"t": t}
            this_steal.update(steal)
            all_steals.append(this_steal)
    ...
    report(c_trades, all_builds, all_steals, n, aidx)
